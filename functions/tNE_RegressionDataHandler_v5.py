...
def open_edges_in_parallel(edge_main_input_dir, backup_edge_main_input_dir, open_func,NJOBS):
    edge_fps = os.listdir(edge_main_input_dir)
    num_edges = len(edge_fps)
    with Pool(NJOBS) as p:
        edge_df_list = p.map(open_func, [(edge_idx, edge_fp, edge_main_input_dir, backup_edge_main_input_dir,num_edges) for edge_idx,edge_fp in enumerate(edge_fps)])
...
def open_edge_fp(args):
    ...
    edge_idx, edge_fp, edge_main_input_dir, backup_edge_main_input_dir,num_edges = args
    ...
    if os.stat(edge_main_input_dir+edge_fp).st_size == 0:
        edge_df = pd.read_csv( backup_edge_main_input_dir + edge_fp)
...
class tNE_RegressionDataHandler:
    ...
    def __init__(self,ALIGN_TAG,eval_tag,pred_type ,infoIDs,x_main_emb_input_dir, y_main_ts_target_input_dir,aux_y_main_ts_target_input_dir,main_output_dir,DESIRED_EVALUATION_TIMESTEP ,
        ...
        self.aux_y_main_ts_target_input_dir = aux_y_main_ts_target_input_dir
    ...
    def load_tNE_reg_data(self, data_type,GET_TUPLE_INFO_DICT=False):
        ...
        for data_period_tuple in data_period_tuples:
            ...
            for infoID in self.infoIDs:
                ...
                #load data
                cur_y_ts_input_dir = self.y_main_ts_target_input_dir + cur_eval_tag + "/" +  hyp_infoID + "/"+ y_tag + "/"
                backup_cur_y_ts_input_dir = self.aux_y_main_ts_target_input_dir + cur_eval_tag + "/" +  hyp_infoID + "/"+ y_tag + "/"
                ydf = open_edges_in_parallel(cur_y_ts_input_dir ,backup_cur_y_ts_input_dir, open_edge_fp,self.NJOBS)
                print()
                print(ydf)
