    ...
    def load_tNE_reg_data(self, data_type):
        ...
        for data_period_tag in data_period_tags:
            ...
            for infoID in self.infoIDs:
                ...
                print()
                print(cur_ss_df[["nodeUserID", "parentUserID"]])




                #filter ydf
                print("\nsize ydf before ss df chop")
                print(ydf.shape)
                print("\nSize after")
                ydf = pd.merge(ydf,cur_ss_df, on=["nodeUserID", "parentUserID"], how="right").reset_index(drop=True)
                ydf = ydf.fillna(0)
                print(ydf.shape)
                ...
                if self.INSERT_TS_FTS == True:
                    ...
                    #update prev dict
                    infoID_to_prev_edge2ts_dict[infoID]=self.convert_ydf_to_edge2ts_dict(ydf)
