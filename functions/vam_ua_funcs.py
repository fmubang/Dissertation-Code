...
def sample_old_users_with_age_v2_SYN_NERF( SYN_NERF,MULT_USER_WEIGHT_BY_AGE, main_cur_all_users_weight_table, num_old_users, weight_col="nodeUserID_overall_action_weight"):

    cur_all_users_weight_table=main_cur_all_users_weight_table.copy()

    #make sure weight is normalized
    cur_all_users_weight_table[weight_col] = cur_all_users_weight_table[weight_col]/cur_all_users_weight_table[weight_col].sum()

    if MULT_USER_WEIGHT_BY_AGE == True:
        cur_all_users_weight_table["user_age"]=cur_all_users_weight_table["user_age"]+1
        cur_all_users_weight_table["normed_age"] = cur_all_users_weight_table["user_age"]/cur_all_users_weight_table["user_age"].sum()
        cur_all_users_weight_table["new_weight"] = cur_all_users_weight_table["normed_age"] * cur_all_users_weight_table[weight_col]
        weight_col = "new_weight"

        print("\nnew weights with age ")
        print(cur_all_users_weight_table[weight_col].value_counts())

    # syn_user_table = cur_all_users_weight_table[cur_all_users_weight_table["nodeUserID"].str.contains("synthetic")].reset_index(drop=True)
    # if syn_user_table.shape[0] > 0:

    if SYN_NERF != 1:
        syn_user_table = cur_all_users_weight_table[cur_all_users_weight_table["nodeUserID"].str.contains("synthetic")].reset_index(drop=True)
        non_syn_user_table = cur_all_users_weight_table[~cur_all_users_weight_table["nodeUserID"].str.contains("synthetic")].reset_index(drop=True)

        print("\nsyn_user_table")
        print(syn_user_table)

        print("\nnon_syn_user_table")
        print(non_syn_user_table)

        #nerf it!
        syn_user_table[weight_col]=syn_user_table[weight_col] * SYN_NERF

        print("\nNERFED syn_user_table")
        print(syn_user_table)

        cur_all_users_weight_table = pd.concat([non_syn_user_table, syn_user_table]).reset_index(drop=True)
        cur_all_users_weight_table[weight_col]=cur_all_users_weight_table[weight_col]/cur_all_users_weight_table[weight_col].sum()

        print("\nnerfed cur_all_users_weight_table")
    ...
    try:
        sampled_old_user_records = cur_all_users_weight_table.sample(n=num_old_users ,weights=cur_all_users_weight_table[weight_col], replace=False)
    except ValueError:
        sampled_old_user_records = cur_all_users_weight_table.sample(n=num_old_users ,replace=False)

    return sampled_old_user_records





def get_user_sim_pred_records_for_cur_infoID_v5_SYN_NERF( infoID_materials_tuple,NUM_LINK_SAMPLE_ATTEMPTS,ADD_OLD_USERS_TO_NEW_USERS_IF_TOO_MANY, timestep, DEBUG_PRINT):

    all_pred_count_dfs_for_cur_infoID_for_verif = []
    all_timestep_dfs_for_cur_infoID = []

    #get infoID materials
    infoID ,hyp_infoID, cur_pred_df, cur_all_users_weight_table, cur_new_users_only_weight_table, cur_all_users_edge_weight_dist_table,cur_new_user_edge_weight_dist_table,num_infoIDs,infoID_idx,CUR_INFO_ID_NEW_USER_COUNTER, MULT_USER_WEIGHT_BY_AGE, SYN_NERF = infoID_materials_tuple
    ...
    if (old_user_diff >= 0):
        ...
        sampled_old_user_records = sample_old_users_with_age_v2_SYN_NERF( SYN_NERF,MULT_USER_WEIGHT_BY_AGE,cur_all_users_weight_table, num_old_users, weight_col="nodeUserID_overall_action_weight")
        ...
        sampled_old_user_records = sample_old_users_with_age_v2_SYN_NERF( SYN_NERF,MULT_USER_WEIGHT_BY_AGE, cur_all_users_weight_table, amount_of_old_users_possible_to_sample, weight_col="nodeUserID_overall_action_weight")
    ...
    if (cur_new_users_only_weight_table.shape[0] > 0) and (cur_new_users_only_weight_table["nodeUserID_overall_influence_weight"].sum()>0):
        initial_new_links = assign_parents_to_new_user_placeholders_v2_debug_print(initial_new_links, cur_new_users_only_weight_table, DEBUG_PRINT)
    elif (cur_new_users_only_weight_table.shape[0] > 0) and (cur_new_users_only_weight_table["nodeUserID_overall_influence_weight"].sum()==0):
        cur_new_users_only_weight_table["nodeUserID_overall_influence_weight"]=1
        initial_new_links = assign_parents_to_new_user_placeholders_v2_debug_print(initial_new_links, cur_new_users_only_weight_table, DEBUG_PRINT)
    else:
        initial_new_links = assign_parents_to_new_user_placeholders_v2_debug_print(initial_new_links, cur_all_users_weight_table, DEBUG_PRINT)
    ...
    return (final_link_records,infoID, CUR_INFO_ID_NEW_USER_COUNTER, cur_pred_df)
...
def create_infoID_to_all_users_weight_table_dict_v3_with_age(history_record_df,INFLUENCE_EPSILON_DIV_VAL,infoIDs):
    ...
    keep_cols = ["nodeUserID", "parentUserID", "informationID","nodeUserID_num_actions_this_timestep","parentUserID_influence_this_timestep", "user_age", "parent_age"]
    ...
    parent_influence_weight_table = user_weight_table[["parentUserID","informationID","parentUserID_overall_influence_weight", "parent_age"]].drop_duplicates().reset_index(drop=True)
    ...
    user_weight_table = user_weight_table.drop_duplicates(["nodeUserID", "informationID"]).reset_index(drop=True)
...
def get_various_ua_tables_v3_with_age(history_record_df, infoIDs,INFLUENCE_EPSILON_DIV_VAL, timestep, SIM_PERIOD, output_dir):
    ...
    #weight tables
    infoID_to_all_users_weight_table_dict =create_infoID_to_all_users_weight_table_dict_v3_with_age(history_record_df,INFLUENCE_EPSILON_DIV_VAL,infoIDs)
    ...
    # new_user_table_hack_tracker_dict = {}
    infoID_hack_track_list = []

    #fix new user table
    for infoID in infoIDs:
        if infoID not in infoID_to_new_users_only_weight_table_dict:
            infoID_to_new_users_only_weight_table_dict[infoID] = infoID_to_all_users_weight_table_dict[infoID].copy()
            infoID_hack_track_list.append(1)
        else:
            infoID_hack_track_list.append(0)

    infoID_hack_track_df = pd.DataFrame(data={"infoID":infoIDs, "used_hack":infoID_hack_track_list})
    infoID_hack_track_df["SIM_PERIOD"]=SIM_PERIOD + 1
    infoID_hack_track_df["sim_period_timestep"] = timestep
    infoID_hack_track_df = infoID_hack_track_df[["infoID", "SIM_PERIOD", "sim_period_timestep", "used_hack"]]

    print("\ninfoID_hack_track_df")
    print(infoID_hack_track_df)

    # hack_output_dir = output_dir + "New-Table-Hack-Tables/"
    # create_output_dir(hack_output_dir)
    # output_fp = hack_output_dir + "SIM_PERIOD-%d-Timestep-%d"%(SIM_PERIOD+1, timestep)
    # infoID_hack_track_df.to_csv(output_fp, index=False)
    # print(output_fp)
    ...
    for infoID in infoIDs:
        ...
        try:
            ...
            print("\nmissing new user table for %s"%infoID)
            continue

    return  edge_weight_df, infoID_to_new_user_edge_weight_dist_table_dict,infoID_to_all_users_edge_weight_dist_table_dict,infoID_to_all_users_weight_table_dict,infoID_to_new_users_only_weight_table_dict,infoID_hack_track_df



def get_test_and_history_tuples(overall_history_start, overall_history_end, overall_test_start, overall_test_end, NUM_SIM_PERIODS, BASIC_GRAN, SIM_PERIOD_SIZE=24):

    time_tuples = []

    overall_history_start = pd.to_datetime(overall_history_start, utc=True)
    overall_history_end = pd.to_datetime(overall_history_end, utc=True)
    overall_test_start = pd.to_datetime(overall_test_start, utc=True)
    overall_test_end = pd.to_datetime(overall_test_end, utc=True)

    #init vars
    cur_history_start = overall_history_start
    cur_history_end = overall_history_end
    cur_test_start = overall_test_start
    cur_test_end = cur_test_start + pd.to_timedelta(SIM_PERIOD_SIZE, unit=BASIC_GRAN) - pd.to_timedelta(1, unit="ns")

    for S in range(NUM_SIM_PERIODS):

        time_tuple = (cur_history_start ,cur_history_end,cur_test_start,cur_test_end)
        time_tuples.append(time_tuple)
        print(time_tuple)

        cur_history_end = cur_history_end + pd.to_timedelta(SIM_PERIOD_SIZE, unit=BASIC_GRAN)
        cur_test_start = cur_test_start + pd.to_timedelta(SIM_PERIOD_SIZE, unit=BASIC_GRAN)
        cur_test_end = cur_test_start + pd.to_timedelta(SIM_PERIOD_SIZE, unit=BASIC_GRAN) - pd.to_timedelta(1, unit="ns")
        # time_tuples.append(time_tuple)

    return time_tuples

def sample_old_users_with_age(MULT_USER_WEIGHT_BY_AGE, main_cur_all_users_weight_table, num_old_users, weight_col="nodeUserID_overall_action_weight"):
...
def get_user_sim_pred_records_for_cur_infoID_v4_old_user_age_sample( infoID_materials_tuple,NUM_LINK_SAMPLE_ATTEMPTS,ADD_OLD_USERS_TO_NEW_USERS_IF_TOO_MANY, timestep, DEBUG_PRINT):

    all_pred_count_dfs_for_cur_infoID_for_verif = []
    all_timestep_dfs_for_cur_infoID = []

    #get infoID materials
    infoID ,hyp_infoID, cur_pred_df, cur_all_users_weight_table, cur_new_users_only_weight_table, cur_all_users_edge_weight_dist_table,cur_new_user_edge_weight_dist_table,num_infoIDs,infoID_idx,CUR_INFO_ID_NEW_USER_COUNTER, MULT_USER_WEIGHT_BY_AGE = infoID_materials_tuple
    ...
    if (old_user_diff >= 0):
        ...
        sampled_old_user_records = sample_old_users_with_age(MULT_USER_WEIGHT_BY_AGE,cur_all_users_weight_table, num_old_users, weight_col="nodeUserID_overall_action_weight")
        ...
        sampled_old_user_records = sample_old_users_with_age(MULT_USER_WEIGHT_BY_AGE, cur_all_users_weight_table, amount_of_old_users_possible_to_sample, weight_col="nodeUserID_overall_action_weight")
        remaining_old_users = num_old_users - amount_of_old_users_possible_to_sample
...
def get_user_sim_pred_records_for_cur_infoID_v3_add_CUR_DATE( infoID_materials_tuple,NUM_LINK_SAMPLE_ATTEMPTS,ADD_OLD_USERS_TO_NEW_USERS_IF_TOO_MANY, timestep, DEBUG_PRINT,CUR_DATE):
    ...
    final_link_records["nodeTime"] = CUR_DATE
    final_link_records["nodeTime"] = pd.to_datetime(final_link_records["nodeTime"], utc=True)
...
def get_user_sim_pred_records_for_cur_infoID_v3_update_volume_counts( infoID_materials_tuple,NUM_LINK_SAMPLE_ATTEMPTS,ADD_OLD_USERS_TO_NEW_USERS_IF_TOO_MANY, timestep, DEBUG_PRINT):
...
def get_various_ua_tables_v2_NEW_USER_TABLE_HACK(history_record_df, infoIDs,INFLUENCE_EPSILON_DIV_VAL, timestep, SIM_PERIOD, output_dir):
    #get edge weight dist tables
    edge_weight_df = history_record_df[["nodeUserID", "parentUserID", "informationID", "edge_weight_this_timestep", "nodeUserID_is_new", "parentUserID_is_new"]]
    infoID_to_new_user_edge_weight_dist_table_dict = get_infoID_to_new_user_edge_weight_dist_table_dict_v2_ua(edge_weight_df)
    infoID_to_all_users_edge_weight_dist_table_dict = get_all_users_edge_weight_dist_table_v2_ua(infoIDs,edge_weight_df)
...
def get_cur_history_record_df_v2_simple(infoIDs, full_history_record_df, history_start, history_end):
    infoID_history_dfs = []
    for infoID in infoIDs:
        #get temp full history
        temp_infoID_history = full_history_record_df[full_history_record_df["informationID"]==infoID].reset_index(drop=True)
        ...
        temp_infoID_history = config_df_by_dates(temp_infoID_history,history_start, history_end, "nodeTime")
        print("\ntemp_infoID_history")
        print(temp_infoID_history)
...
def get_lookback_dates_per_infoID_over_time(main_lookback_over_time_df, test_start, test_end, num_test_timesteps, MIN_LB_SCALING_FACTOR, BASIC_GRAN, infoIDs):
    ...
    for infoID in infoIDs:
        ...
        lookback_over_time_df["history_end"] =lookback_over_time_df["nodeTime"] - pd.to_timedelta(1, unit="ns")
...
def get_infoID_material_list_v3_SYN_NERF(infoIDs, NEW_USER_COUNTER_DICT,hyp_dict,cleaned_pred_df_dict,timestep,infoID_to_all_users_weight_table_dict,
    infoID_to_new_users_only_weight_table_dict,infoID_to_all_users_edge_weight_dist_table_dict,infoID_to_new_user_edge_weight_dist_table_dict,MULT_USER_WEIGHT_BY_AGE, SYN_NERF):
    ...
    for infoID_idx, infoID in enumerate(infoIDs):
        ...
        infoID_materials_tuple = (infoID ,hyp_infoID, cur_pred_df, cur_all_users_weight_table, cur_new_users_only_weight_table,
            cur_all_users_edge_weight_dist_table,cur_new_user_edge_weight_dist_table,num_infoIDs,infoID_idx,CUR_INFO_ID_NEW_USER_COUNTER,MULT_USER_WEIGHT_BY_AGE, SYN_NERF)

        infoID_materials_list.append(infoID_materials_tuple)

    return infoID_materials_list

def get_infoID_material_list_v2_more_params(infoIDs, NEW_USER_COUNTER_DICT,hyp_dict,cleaned_pred_df_dict,timestep,infoID_to_all_users_weight_table_dict,
    ...
    for infoID_idx, infoID in enumerate(infoIDs):
        ...
        infoID_materials_tuple = (infoID ,hyp_infoID, cur_pred_df, cur_all_users_weight_table, cur_new_users_only_weight_table,
            cur_all_users_edge_weight_dist_table,cur_new_user_edge_weight_dist_table,num_infoIDs,infoID_idx,CUR_INFO_ID_NEW_USER_COUNTER,MULT_USER_WEIGHT_BY_AGE)
...
def get_weight_start_and_weight_end_lists(TEST_START_DATE, HISTORICAL_LOOKBACK_DAYS_LIST):
    ...
    CUR_DAY = TEST_START_DATE - pd.to_timedelta(1, unit="ns")
    print("\nCUR_DAY")
    print(CUR_DAY)
...
def get_infoID_to_old_user_df_dict(df, infoIDs,history_start,history_end, GRAN, platform):
    ...
    infoID_to_old_user_df_dict = {}
    print("\ninfoID_to_old_user_df_dict")
    print(infoID_to_old_user_df_dict)
    for infoID in infoIDs:
        cur_infoID_df = df[df["informationID"]==infoID].reset_index(drop=True)
        infoID_to_old_user_df_dict[infoID]=cur_infoID_df
        print()
