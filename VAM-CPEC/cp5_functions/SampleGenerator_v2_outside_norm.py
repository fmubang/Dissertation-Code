...
class SampleGenerator:
    ...
    def __init__(self,train_start,train_end,val_start,val_end,test_start,test_end,GRAN,
        ...
        CORRELATION_FUNCS_TO_USE_LIST,CORRELATION_STR_TO_FUNC_DICT,AGG_TIME_SERIES, DARPA_INPUT_FORMAT, GET_NEWS_DATA):
        ...
        more_param_vals = [RESCALE_TARGET,SCALER_TYPE,FEATURE_RANGE,INPUT_TWITTER_LOGNORM,OUTPUT_TWITTER_LOGNORM,INPUT_OTHER_LOGNORM,OUTPUT_OTHER_LOGNORM,LOGNORM_IGNORE_TAGS,GET_1HOT_INFO_ID_FTS, str(CORRELATION_FUNCS_TO_USE_LIST),AGG_TIME_SERIES,DARPA_INPUT_FORMAT]
        more_param_names = ["RESCALE_TARGET","SCALER_TYPE","FEATURE_RANGE","INPUT_TWITTER_LOGNORM","OUTPUT_TWITTER_LOGNORM","INPUT_OTHER_LOGNORM","OUTPUT_OTHER_LOGNORM","LOGNORM_IGNORE_TAGS","GET_1HOT_INFO_ID_FTS", "CORRELATION_FUNCS_TO_USE_LIST","AGG_TIME_SERIES","DARPA_INPUT_FORMAT"]
    ...
    def normalize_train_data_and_return_scalers_v2_OUTSIDE_DATA(self, outside_x_train, outside_y_train):

        outside_x_train = self.log_norm_x_arrays(outside_x_train)
        outside_y_train = self.log_norm_y_arrays(outside_y_train)

        self.x_scaler=None
        if self.RESCALE == True:
            outside_x_train,self.x_scaler = normalize_single_train_only_data_v2_standard_option(outside_x_train, self.SCALER_TYPE,feature_range=self.FEATURE_RANGE)

        self.y_scaler=None
        if self.RESCALE_TARGET == True:
            outside_y_train,self.y_scaler = normalize_single_train_only_data_v2_standard_option(outside_y_train, self.SCALER_TYPE,feature_range=self.FEATURE_RANGE)

        return outside_x_train,outside_y_train,self.x_scaler,self.y_scaler
